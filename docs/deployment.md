...
The dashboard onboarding baseline (`8001c5047d0e29c62f3708dbe5524abd327a521b`) and the widget search presence release (`7a8b7c80d88cf8259449d79093a070b02be3f107`) are deployed to Hostinger. The health endpoint returned database ready. The production catalog has one active realtime chat service and one active zero-price Free plan, seeded by `ProductionPlanSeeder`. The server-side deploy helper `/home/u320743426/deploy-myvivahai.sh` runs that idempotent seeder after migrations. Live Reverb delivery has not been verified; keep `CHAT_REALTIME_ENABLED=false` on shared hosting until tested. The presence sweep is **not yet scheduled**: add a once-per-minute job in Hostinger hPanel Cron Jobs using `/usr/bin/php /home/u320743426/domains/digitalji.in/public_html/myvivahai/artisan chat:presence-sweep`. The SSH `crontab` wrapper on this account only reads the current crontab and cannot install jobs.

A live probe of the previously uploaded Disha endpoint returned zero eligible matches for multiple search terms. The supplied Disha registration code creates new profiles with `profile_approve = No`, while the search config correctly limits results to active, visible, approved profiles. Do not broaden search to pending/private profiles without a Disha policy decision. The local Disha package now normalizes that predicate and performs case-insensitive name/ID search. Its config credential was refreshed and synced to the encrypted MyVivahAI integration; **upload the current local Disha config and endpoint files before retesting**, or the old Disha credential will no longer match. MyVivahAI search cards also expose online/offline from widget heartbeats; this reports chat-widget presence, not Disha site login state.
