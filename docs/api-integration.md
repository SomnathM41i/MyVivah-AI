...
**Current status:** platform PASETO auth, key lifecycle, audit, chat APIs and platform rate limiting are implemented. The dashboard has a user-search endpoint/authentication form and a connection test using an operator-supplied requester/query, backed by the existing `platform_integrations` row. Credentials are encrypted; only a configured/not-configured flag is returned through the API. Search behavior is specified in [external-user-search-v1.md](external-user-search-v1.md). Test/live environments, general capability editing, and test logs are not implemented.
